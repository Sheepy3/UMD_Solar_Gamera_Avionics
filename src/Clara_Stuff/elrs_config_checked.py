"""Standalone ELRS parameter tool with checked chunk assembly/readback.

The known-working elrs_config.py is intentionally left unchanged beside this
...
def parse_frames(buffer: bytearray) -> list[tuple[int, int, bytes]]:
    ...
    while len(buffer) >= 4:
        ...
        if crc8_d5(raw[2:-1]) != raw[-1]:
            continue

        frames.append((raw[0], raw[2], raw[3:-1]))
...
class ElrsConfigurator:
    def __init__(
        self, serial_port, frame_address: int, device_id: int, handset_id: int
    ):
    ...
    def read_all_parameters(self, field_count: int) -> list[ParameterField]:
        ...
        for field_id in range(1, field_count + 1):
            parameter = self.read_parameter(field_id)
    ...
    def write_parameter(self, param: ParameterField, value: int) -> None:
        payload = bytes(
            [self.device_id, self.handset_id, param.id, value & 0xFF]
        )
...
def decode_parameter(field_id: int, data: bytes) -> ParameterField | None:
    ...
    param = ParameterField(
        id=field_id, parent=parent, type=field_type, name=name
    )
...
def find_field(
    fields: list[ParameterField], wanted_name: str
) -> ParameterField | None:
    ...
    for parameter in fields:
        if parameter.name.casefold() == wanted:
            return parameter
    for parameter in fields:
        if wanted in parameter.name.casefold():
            return parameter
    return None
...
def print_fields(fields: list[ParameterField]) -> None:
    for parameter in fields:
        if parameter.type == FIELD_TEXT_SELECTION:
            current = ""
            if (
                parameter.value is not None
                and parameter.value < len(parameter.values)
            ):
                current = f" current={parameter.values[parameter.value]!r}"
            options = ", ".join(
                repr(value) for value in parameter.values if value
            )
            print(
                f"{parameter.id:02d}: {parameter.name} [select]"
                f"{current} options=[{options}]"
            )
        else:
            print(
                f"{parameter.id:02d}: {parameter.name} "
                f"type={parameter.type} value={parameter.value}"
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Checked PC-side version of the ELRS Lua parameter reader/writer."
        )
    )
    parser.add_argument("--port", default="COM5")
    parser.add_argument("--baud", type=int, default=921600)
    parser.add_argument(
        "--frame-address", type=parse_int, default=ADDR_TX_MODULE
    )
    ...
    parser.add_argument(
        "--list", action="store_true", help="List all readable TX parameters."
    )
    parser.add_argument(
        "--set",
        nargs=2,
        metavar=("FIELD", "OPTION"),
        help="Set a text-select field by name.",
    )
    parser.add_argument(
        "--packet-rate", help="Shortcut for --set 'Packet Rate' OPTION."
    )
    parser.add_argument(
        "--switch-mode", help="Shortcut for --set 'Switch Mode' OPTION."
    )
    return parser.parse_args()


class CheckedElrsConfigurator(ElrsConfigurator):
    def read_parameter(
        self, field_id: int, timeout: float = 2.5
    ) -> ParameterField | None:
        """Read one field while rejecting stale or duplicate chunks."""
        chunks = bytearray()
        chunk_index = 0
        expected_chunks_remaining: int | None = None
        ...
        while time.monotonic() < deadline:
            ...
            for _, frame_type, payload in self.collect_frames(0.12):
                ...
                # Extended response payload: destination handset, origin
                # device, field ID, chunks remaining, then field bytes.
                if (
                    payload[0] != self.handset_id
                    or payload[1] != self.device_id
                    or payload[2] != field_id
                ):
                    continue

                chunks_remaining = payload[3]
                if (
                    expected_chunks_remaining is not None
                    and chunks_remaining != expected_chunks_remaining
                ):
                    # A late duplicate from the prior request would otherwise
                    # be appended twice and shift the decoded current value.
                    continue
                ...
                expected_chunks_remaining = chunks_remaining - 1
                chunk_index += 1
                break

            # If only unrelated/stale frames arrived, request the same chunk
            # again instead of advancing or appending anything.
...
def describe_selection(param: ParameterField | None) -> str:
    if param is None:
        return "no readback response"
    if param.type != FIELD_TEXT_SELECTION:
        return f"unexpected field type {param.type}"
    if param.value is None:
        return "readback has no current value"
    if param.value >= len(param.values):
        return f"readback index {param.value} is outside the option list"
    return f"current={param.values[param.value]!r} (index {param.value})"


def discover_and_read(
    configurator: CheckedElrsConfigurator,
) -> tuple[DeviceInfo, list[ParameterField]]:
    configurator.serial.reset_input_buffer()
    info = configurator.discover_tx()
    if info is None:
        raise RuntimeError(
            "No ELRS TX device info response. Stop Clara/headless_tx, make "
            "sure the TX is on this port, and try --frame-address 0xC8 if "
            "0xEE does not respond."
        )
    ...
    return info, configurator.read_all_parameters(info.field_count)
...
def main() -> None:
    ...
    with serial.Serial(
        args.port, args.baud, timeout=0, write_timeout=0.5
    ) as serial_port:
        configurator = CheckedElrsConfigurator(
            serial_port,
        ...
        _, fields = discover_and_read(configurator)
        if args.list:
            print_fields(fields)
        ...
        for field_name, option_name in requested_sets:
            ...
            if param is None:
                raise RuntimeError(
                    f"Could not find field named {field_name!r}. "
                    "Run --list to inspect names."
                )
            if param.type != FIELD_TEXT_SELECTION:
                raise RuntimeError(
                    f"Field {param.name!r} is not a text-selection field."
                )

            option_index = find_option(param, option_name)
            if option_index is None:
                raise RuntimeError(
                    f"Could not find option {option_name!r} for "
                    f"{param.name!r}. Options: {param.values}"
                )

            configurator.write_parameter(param, option_index)
            print(
                f"Set {param.name!r} to "
                f"{param.values[option_index]!r} ({option_index})."
            )

            # The official Lua workflow waits briefly for settings storage,
            # then queues the changed field for a fresh parameter read.
            time.sleep(0.3)
            readback = configurator.read_parameter(param.id)
            print(f"Read back {param.name!r}: {describe_selection(readback)}")

            if readback is not None and readback.value != option_index:
                print(
                    "WARNING: requested index "
                    f"{option_index}, module reported {readback.value}."
                )
